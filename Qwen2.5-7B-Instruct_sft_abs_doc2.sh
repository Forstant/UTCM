#!/bin/bash
user_name=lyx_doc_2_abs
purpose='doc2的abs'
export CUDA_VISIBLE_DEVICES=0
model_name=Qwen2.5-7B-Instruct
method=sft
model_config_name=${model_name}_sft_doc_2_abs.yaml
...
#             user_name=lyx_doc_2_abs_${abs}
#             sft_file_name=doc_2_abs_${abs}
...
for abs in ${abs_list[@]}; do
    for lr in ${lr_list[@]}; do
        for warmup_ratio in ${warmup_ratio_list[@]}; do
            user_name=lyx_doc_2_abs_${abs}
            sft_file_name=doc_2_abs_${abs}
            # 如果abs为0，lr为1e-5或者5e-5或5e-6，则跳过
            if [[ $abs -eq 0 && ($lr == 1.0e-5 || $lr == 5.0e-5 || $lr == 5.0e-6) ]]; then
                continue
            fi
