...
for lr in ${lr_list[@]}; do
    for warmup_ratio in ${warmup_ratio_list[@]}; do
        ...
        sed -i "s|^\(dataset: \).*$|\1sft_train_2|" "${save_path}/${model_config_name}"
        sed -i "s|^\(max_samples: \).*$|\115000|" "${save_path}/${model_config_name}"
        # 首先在文件中写入名字和目的
