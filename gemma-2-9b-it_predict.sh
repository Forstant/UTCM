...
# 在最后面加上当前时间
sed -i "s|^\(output_dir: \).*$|\1outputs/saves/${model_name}/${method}/${current_time}|" 'configs/predict/gemma-2-9b-it_predict.yaml'
# 首先在文件中写入名字和目的
echo "用户：$user_name" >> "$log_file"
echo "目的：$purpose" >> "$log_file"
# 将配置文件的内容输出到日志文件中
cat 'configs/predict/gemma-2-9b-it_predict.yaml' >> "$log_file"
llamafactory-cli train 'configs/predict/gemma-2-9b-it_predict.yaml' &>> "$log_file"
